...
Validation Layers (Priority Order):
  ...
  4. Sunglasses Detection       (4-signal voting — inline, no external file)
...
# [F2] Sunglasses — relaxed thresholds, 4-signal vote
SG_BRIGHTNESS_MAX   = 80    # was 55
SG_CONTRAST_MAX     = 35    # was 20
SG_HSV_V_MAX        = 85    # new
SG_EDGE_DENSITY_MIN = 0.08  # new
SG_VOTE_THRESHOLD   = 3     # needs 3 / 4
...
# ============================================================
# MODULE 3 — Sunglasses Detection  (4-signal vote, inline)  [F2]
# ============================================================
def detect_sunglasses(frame_cv2: np.ndarray, face_landmarks) -> dict:
    """
    4 independent signals, needs SG_VOTE_THRESHOLD (3/4) to confirm.

    Signal 1 — Brightness  : dark lens → low mean grey
    Signal 2 — Contrast    : dark lens → low std
    Signal 3 — HSV Value   : dark lens → low V channel
    Signal 4 — Edge density: frame outline → higher edge density

    Thresholds relaxed vs original to catch reflective / tinted lenses.
    """
    ...
    for side, roi in [("left", _roi(SG_LEFT_EYE_POLY)),
        ...
        if roi.size == 0:
            ...
            details[f"{side}_edge_density"] = 0.0
            continue
        ...
        details[f"{side}_edge_density"] = round(
            float(np.sum(edges > 0)) / (edges.size + 1e-8), 4
        )
    ...
    l_e = details.get("left_edge_density",   0)
    r_e = details.get("right_edge_density",  0)

    votes = 0

    if l_b < SG_BRIGHTNESS_MAX and r_b < SG_BRIGHTNESS_MAX:
        votes += 1
        details["s1_brightness"] = f"sunglasses (L={l_b}, R={r_b})"
    else:
        details["s1_brightness"] = f"clear (L={l_b}, R={r_b})"

    if l_c < SG_CONTRAST_MAX and r_c < SG_CONTRAST_MAX:
        votes += 1
        details["s2_contrast"] = f"sunglasses (L={l_c}, R={r_c})"
    else:
        details["s2_contrast"] = f"clear (L={l_c}, R={r_c})"

    if l_v < SG_HSV_V_MAX and r_v < SG_HSV_V_MAX:
        votes += 1
        details["s3_hsv_v"] = f"sunglasses (L={l_v}, R={r_v})"
    else:
        details["s3_hsv_v"] = f"clear (L={l_v}, R={r_v})"

    if l_e > SG_EDGE_DENSITY_MIN and r_e > SG_EDGE_DENSITY_MIN:
        votes += 1
        details["s4_edges"] = f"sunglasses (L={l_e}, R={r_e})"
    else:
        details["s4_edges"] = f"clear (L={l_e}, R={r_e})"

    detected        = votes >= SG_VOTE_THRESHOLD
    details["votes"] = votes
    reason          = (
        f"Sunglasses detected ({votes}/4 signals)"
        if detected else
        f"No sunglasses ({votes}/4 signals)"
    )
